...
# ---------------- HTML -> text (stdlib, deterministic) ----------------
...
def extract_primary_10k_text(combined: str) -> Optional[str]:
    """
    From a combined SEC filing (SGML), find the 10-K (or 10-K/A) document's TEXT block.
    ...
    for m in _DOC_RE.finditer(combined):
        ...
        if doctype not in {"10-K", "10-K/A"}:
            continue
        mtext = _TEXT_RE.search(block)
        raw = mtext.group(1) if mtext else block
...
def iter_filings(subs: dict,
                 form_types: Iterable[str],
                 start_date: Optional[dt.date],
                 end_date: Optional[dt.date]) -> Iterator[Tuple[str, dt.date, str]]:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Bulk download SEC 10-K filings and (optionally) extract plain text.")
    ...
    p.add_argument("--forms", nargs="*", default=["10-K", "10-K/A"], help="Form types (default: 10-K,10-K/A)")
    p.add_argument("--start-date", type=str, help="YYYY-MM-DD inclusive")
    p.add_argument("--end-date", type=str, help="YYYY-MM-DD inclusive")
    p.add_argument("--outdir", type=str, default="sec_10k", help="Output root directory")
    ...
    p.add_argument("--extract-text", action="store_true", help="Also write plain-text file from 10-K document")
    return p.parse_args()
            ...
            if args.extract_text:
                try:
                    with open(raw_path, "r", encoding="utf-8", errors="ignore") as f:
                        combined = f.read()
                    text = extract_primary_10k_text(combined)
                    if text:
                        with open(plain_path, "w", encoding="utf-8") as f:
                            f.write(text + "\n")
                        print(f"[TXT ] Wrote plain text: {plain_path}")
                    else:
                        print(f"[WARN] No 10-K TEXT block found in: {raw_path}")
